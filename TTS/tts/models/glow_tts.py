...
class GlowTts(nn.Module):
    ...
    def __init__(self,
        ...
        print('self.c_in_channels: ', self.c_in_channels)
        ...
        if 2 > 1 and not external_speaker_embedding_dim:
        # speaker embedding layer
            self.emb_g = nn.Embedding(1, self.c_in_channels)
            print('self.emb_g.weight in the if statement: ', self.emb_g.weight)
    ...
    def inference(self, x, x_lengths, g=None):
        if g is not None:
            if self.external_speaker_embedding_dim:
                ...
                print('else statement was called')
                print('self.emb_g: ', self.emb_g)
                print('self.emb_g(g): ', self.emb_g(g))
                print('F.normalize(self.emb_g(g)): ', F.normalize(self.emb_g(g)))
